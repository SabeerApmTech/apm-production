...
} from "@/store/services/reworkProductionHistoryApi"
...
function LogsPanel({ reworkScheduleId, sequenceNo }: LogsPanelProps) {
  const { data, isLoading } = useGetReworkProductionHistoryLogsQuery({ reworkScheduleId, sequenceNo })
...
export function ScheduleOperationsDetail({ reworkScheduleId, onHeightChange }: ScheduleOperationsDetailProps) {
  const { data, isLoading } = useGetReworkProductionHistoryOperationsQuery(reworkScheduleId)
        ...
        <table className="min-w-max w-full text-sm">
          ...
          <tbody>
            {data.map((op) => {
              ...
              return (
                <Fragment key={op.reworkScheduleOperationId}>
                  ...
                  {isOpen && (
                    <tr>
                      <td colSpan={6} className="bg-white p-0">
                        <LogsPanel reworkScheduleId={reworkScheduleId} sequenceNo={op.sequenceNo} />
                      </td>
                    </tr>
                  )}
                </Fragment>
