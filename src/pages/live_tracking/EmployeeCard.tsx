...
const STATUS_CFG: Record<EmployeeStatus, { dot: string; text: string }> = {
  ...
  Stopped:       { dot: "bg-red-500",   text: "text-red-500 dark:text-red-400"   },
  "Not Started": { dot: "bg-gray-400",  text: "text-muted-foreground"  },
}
...
export function EmployeeCard({ emp, onViewDetail }: Props) {
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-amber-200 bg-amber-50/40 shadow-sm overflow-hidden dark:border-amber-900/60 dark:bg-amber-950/10">
      <div className="flex flex-col gap-1.5 p-4">
        <Row label="Employee Name">
          <span className="font-semibold text-foreground">{emp.name}</span>
        </Row>
        <Row label="Employee ID">
          <span className="font-medium text-foreground">{emp.employeeId}</span>
        </Row>
        <Row label="Schedule ID">
          <span className="font-medium text-foreground">{emp.scheduleId}</span>
        </Row>
        <Row label="Company">
          <span className="font-semibold text-foreground">{emp.company}</span>
        </Row>
        <Row label="Product">
          <span className="font-semibold text-foreground">{emp.product}</span>
        </Row>
        <Row label="Step">
          <span className="font-semibold text-foreground">{emp.step}</span>
        </Row>
        <Row label="Target Qty">
          <span className="font-medium text-foreground">{emp.targetQty.toLocaleString()}</span>
        </Row>
        <Row label="Progress">
