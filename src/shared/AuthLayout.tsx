...
interface AuthLayoutProps {
  /** Extra classes for the split card — e.g. sizing, since Login and OperatorLogin size it differently. */
  cardClassName?: string
  /** Extra classes for the left (form/content) panel. */
  leftClassName?: string
  cornerLabel: string
  cornerTo: string
  /** Rendered on top of the right-side product image — e.g. OperatorLogin's gradient + caption. */
  rightOverlay?: ReactNode
  children: ReactNode
}

// Shared outer shell for the two login screens: full-viewport centered wrapper, split card,
// top-right corner nav button, and the right-side product image panel. Each page still owns its
// own left-panel content (logo, heading, form) since that part differs meaningfully between them.
